...
def extract_labels(fname, cnt, one_hot=False, num_classes=10):
  ...
  """
  with open(fname, "rb") as bytestream:
    buf = bytestream.read(cnt * 4)
    dt = numpy.dtype(numpy.uint32).newbyteorder('>')
    labels = numpy.frombuffer(buf, dt)
    if one_hot:
...
def read_data_sets(cnt,limit,
  ...
  train_labels = extract_labels('/tf/data/labels.dat', cnt, one_hot=one_hot, num_classes=limit)
